...
DEFINE_I_TYPED_TRANCEIVER(img, img) // transceiver interface 
DEFINE_C_TYPED_QUEUE(img, img) // channel

behavior DUT(i_img_receiver im, i_img_sender ed) {      
...
behavior Main
{
   ...
   Platform canny(Cin, Cout);
   Monitor monitor(Cout);

   int main(void) {
      par { stimulus; canny; monitor; } 
      return 0;
   }
};
